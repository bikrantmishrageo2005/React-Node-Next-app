...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-black/90 border border-neon-cyan p-3 rounded shadow-[0_0_10px_rgba(0,243,255,0.3)] z-50">
        <p className="text-neon-cyan font-orbitron text-xs mb-1 tracking-wide">{label}</p>
...
export default function PollutionCharts({ breakdown, forecast }: { breakdown: PollutionData[], forecast: any[] }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
      ...
      >
        <h3 className="text-neon-purple font-orbitron text-lg mb-6 tracking-wider">POLLUTION VECTOR ANALYSIS</h3>
        <div className="flex-1 w-full h-full min-h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={breakdown}>
              <PolarGrid stroke="#333" />
              <PolarAngleAxis dataKey="type" tick={{ fill: '#9ca3af', fontSize: 11, fontFamily: 'Rajdhani', dy: 4 }} />
      ...
      <motion.div 
        ...
        className="glass-panel p-6 rounded-xl min-h-[400px] flex flex-col"
      >
        <h3 className="text-neon-cyan font-orbitron text-lg mb-6 tracking-wider">24H PREDICTIVE MODEL</h3>
        <div className="flex-1 w-full h-full min-h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={forecast} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="time" stroke="#333" tick={{ fill: '#666', fontSize: 10 }} tickLine={false} axisLine={false} />
